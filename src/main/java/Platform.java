...
public class Platform implements IPlatform {
    ...
    public Platform(IProtocolFactory factory) {
    ...
    public void createVirtualNode(IPhysicalNode physicalNode) {
        String id = UUID.randomUUID().toString();
        IVirtualNode node = new VirtualNode(id, factory.createProtocol(id));
    ...
    public void registerVirtualNode(String id, IVirtualNode node) {
        if (!registeredVirtualNodes.containsKey(id)) {
            ...
            node.registerOtherNodes(new ArrayList<String>(this.getRegisteredVirtualNodes().keySet()));
        }
    }
    ...
    public void run() {
        int tick = 0;
        while (tick < 2000) {
            for (IPhysicalNode currentNode: registeredPhysicalNodes.values()) {
                System.out.format("Tick %d. Giving control to node with id %s \n", tick, currentNode.getNodeId());
                runStep(currentNode.getNextVirtualNode());
                tick++;
            }
        }
    }
